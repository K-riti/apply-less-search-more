...
from .fetch import Job, fetch_all
from .mock import fetch_all_mock
from .prefilter import REMOTE_HINTS, prefilter
...
def cmd_run(args) -> int:
    ...
    if not jobs:
        print("  no unseen jobs this run; will backfill digest from tracked high-fit roles")
    ...
    if jobs:
        ...
        print("\n[3/5] screening skipped (no unseen jobs)")
    ...
    loc_pref = [str(l).lower() for l in (filters.get("locations") or [])]

    def _location_rank(job) -> int:
        hay = f"{job.location} {job.title}".lower()
        if any(h in hay for h in REMOTE_HINTS):
            return 0
        for i, token in enumerate(loc_pref, start=1):
            if token in hay:
                return i
        return len(loc_pref) + 1

    scored = [j for j in jobs if (j.score or 0) >= threshold]
    shortlist = sorted(scored, key=lambda j: (_location_rank(j), -(j.score or 0)))[:top_n]
    ...
    # Backfill from tracked high-fit roles to hit daily target when fresh jobs
    # are sparse. This keeps output actionable without lowering fit quality.
    if len(shortlist) < top_n:
        need = top_n - len(shortlist)
        seen_ids = {j.job_id for j in shortlist}
        backlog = []
        for jid, row in sorted(store.data.items(), key=lambda kv: kv[1].get("first_seen", ""), reverse=True):
            if jid in seen_ids or row.get("applied"):
                continue
            try:
            ...
            if len(backlog) >= need:
                break
        if backlog:
            shortlist.extend(backlog)
            print(f"  backfilled {len(backlog)} tracked high-fit jobs to reach daily target")
